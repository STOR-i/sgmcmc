...
```
Optionally we can set the tuning parameter for the momentum `alpha` in the same way as the `stepsize`. But we'll leave this along with the trajectory integer `L`, and the `minibatchSize` as their defaullts, 3 and `0.01 * N = 100` respectively.
...
```{r message=FALSE}
chains = sghmc( logLik, dataset, params, stepsize, logPrior = logPrior )
```
If you don't specify the `logPrior` an uninformative uniform prior will be used.
