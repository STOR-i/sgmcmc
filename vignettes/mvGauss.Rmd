...
```{r message=FALSE}
chains = sgld( logLik, logPrior, dataset, params, stepsize, n, nIters = 10^4 )
```
...
There are lots of other sgmcmc algorithms implemented that can be used by simply changing the tuning parameters slightly including `sghmc` and `sgnht`; as well as their [control variate counterparts](https://arxiv.org/pdf/1705.05439.pdf) for improved efficiency.
