...
Now let's declare the `logLik` and `logPrior` functions. Remember that for ease of use, all distribution functions located in `tf$contrib$distributions` have been copied to `tf$distributions` as well (for more details see the [Get Started](https://stor-i.github.io/sgmcmc///articles/sgmcmc.html) page).
```{r}
