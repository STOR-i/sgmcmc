...
# Introduction to sgmcmc: A package for stochastic gradient MCMC
...
```
You can specify as many datasets as you like, the most important thing is that your naming is consistent, so you use the same names in your log likelihood and log prior functions as how you specified them here.

The functions assume that each observation is located on the first axis of the object. Suppose `Y` is a 2d matrix, so observation $Y_i$ should be located at `dataset$Y[i,]`. Similarly if `X` is a 3d array, observation $X_i$ should be located at `dataset$X[i,,]`.
...
Suppose we want to simulate from the mean of a multivariate Normal density with the each component of the mean having a Student T prior, we would specify this as follows
```{r eval=FALSE}
library(MASS)
...
logPrior = function(params) {
    ...
    distn = tf$contrib$distributions$StudentT(df = 3)
